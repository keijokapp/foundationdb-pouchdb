/* eslint-disable camelcase,no-underscore-dangle,stylistic/max-len */
import {
	collate,
	toIndexableString,
	normalizeKey,
...
export async function query(db, fun, opts) {
	if (typeof fun !== 'string') {
		return getQueue('temp_view/temp_view').add(async () => {
			const view = await createView(
				/* sourceDB */ db,
				'temp_view/temp_view',
				typeof fun === 'function' ? fun : fun.map,
				typeof fun === 'function' ? undefined : fun.reduce
			);
	...
	const [designDocName, viewName] = parts;
	const fullViewName = parts.join('/');
	...
	return getQueue(fullViewName).add(async () => {
		...
		const view = await createView(
			/* sourceDB */ db,
			fullViewName,
			doc.views[viewName].map,
			doc.views[viewName].reduce
		);
