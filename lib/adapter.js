...
export default function FoundationdbAdapter(api, { db, name, revs_limit: revLimit }) {
	...
	const stores = {
		...
		docStore: /** @type {fdb.Subspace<Id, Id, Metadata, Metadata>} */(
	...
	api._bulkDocs = callbackify(
		...
		({ docs: userDocs }, opts) => doWriteTn(async tn => {
			...
			tn.at(stores.updateSeqStore).set(emptyBuffer, newUpdateSeq);
			tn.at(stores.docCountStore).set(emptyBuffer, docCount + docCountDelta);
