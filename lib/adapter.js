...
const metadataEncoder = {
	...
	packUnboundVersionstamp(metadata) {
		...
		metadata.seq = /** @type {any} */(revSeq.toString(16).padStart(4, '0'));
		...
		for (const key in metadata.rev_map) {
			...
			if (typeof revSeq === 'bigint') {
				...
				metadata.rev_map[rev] = /** @type {any} */(revSeq.toString(16).padStart(4, '0'));
			}
		}
...
const maxSeq = /** @type {import('./types.js').SeqString} */('ffffffffffffffffffffffff');
 ...
 *   | fdb.Transaction
 * )} Actionable
 * @typedef {{ queue: Deque<Task>, seqOffset: number }} TransactionState
 * @typedef {{
...
/**
 * @template {number | import('./types.js').SeqString} Seq
 * @param {any} api
...
export default function FoundationdbAdapter(api, {
	...
	/**
	 * @template T
	 * @param {(tn: fdb.Transaction, state?: TransactionState) => Promise<T>} fn
	 * @returns {Promise<T>}
	 */
	...
	/**
	 * @param {import('./types.js').SeqString} seq
	 * @return {Seq}
	 */
	function toSeq(seq) {
		if (sparseSeq) {
		...
		return !Number.isInteger(seqNumber) || seqNumber > Number.MAX_SAFE_INTEGER
			? /** @type {Seq} */(seq)
			: /** @type {Seq} */(seqNumber);
	}
	...
	const stores = {
		...
		docStore: /** @type {fdb.Subspace<import('./types.js').Id | typeof startOfKeyspace | typeof endOfKeyspace, import('./types.js').Id, import('./types.js').Metadata, import('./types.js').Metadata>} */(
			subspace.at(fdb.tuple.pack(DOC_STORE), idEncoder, metadataEncoder)
		),
		...
		docCountStore: subspace.at(fdb.tuple.pack(DOC_COUNT_KEY), fdb.encoders.buf, int32LEEncoder),
	...
	api._info = callbackify(
		...
		async () => doReadTn(async tn => {
			const [docCount = 0, updateSeq = minSeq] = await Promise.all([
				...
				sparseSeq
					? tn.getReadVersion().then(v => /** @type {import('./types.js').SeqString} */(`${v.toString('hex')}ffffffff`))
	...
	api._get = callbackify(
		...
		(id, { rev, latest } = {}) => doReadTn(async tn => {
			...
			// Some type shenanigans to avoid unnecessary copying
			metadata.seq = /** @type {any} */(toSeq(metadata.seq));
	...
	api._bulkDocs = callbackify(
		 ...
		 */
		({ docs: userDocs }, opts) => doWriteTn(async (tn, state) => {
			...
			let docCountDelta = 0;
			// not used in sparse mode
			const updateSeq = sparseSeq
				? 0n
				: BigInt(`0x${await tn.at(stores.updateSeqStore).get(emptyBuffer) ?? 0}`);
			// not used in sparse+transaction mode
			let updateSeqOffset = 0;
			...
			const results = await Promise.allSettled(docInfos.map(async docInfo => {
				...
				if (sparseSeq) {
					const seq = state ? ++state.seqOffset : ++updateSeqOffset;

					// Type shenanigans to avoid extra variables, copying etc.
					// These _would_ be baked (fixed) after the after transaction commit
					// if baking was is requested (which it is not).
					docInfo.metadata.seq = /** @type {any} */(seq);
					docInfo.metadata.rev_map[rev] = /** @type {any} */(seq);
					tn.at(stores.bySeqStore).setVersionstampedKey(seq, docInfo.data, false);
					tn.at(stores.docStore).setVersionstampedValue(id, docInfo.metadata, false);
				} else {
					const seq = updateSeq + BigInt(++updateSeqOffset);
			...
			if (sparseSeq) {
				// We don't really need to keep track of seq in the sparse mode but there are a couple of
				// reasons to do it:
				// 1. There needs to be some that's updated on each mutation so that live changes works.
				// 2. Even though it's not really supported, it should make it possible to switch
				//    to non-sparse mode.
				tn.at(stores.updateSeqStore).setVersionstampedValue(
					emptyBuffer,
					state ? state.seqOffset : updateSeqOffset,
					false
				);
			} else {
				tn.at(stores.updateSeqStore).set(emptyBuffer, updateSeq + BigInt(updateSeqOffset));
			}
