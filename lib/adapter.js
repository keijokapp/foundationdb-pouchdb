...
/**
 * @import { Transformer } from 'foundationdb/dist/lib/transformer.js'
 * @import {
 *   AllDocsResult,
 *   AllDocsResultRow,
 *   AllDocsResultRowDoc,
 ...
 * @type {Transformer<Id | typeof startOfKeyspace | typeof endOfKeyspace, Id>}
 */
const idEncoder = {
	pack(id) {
	...
	unpack(idBuffer) {
		...
		return /** @type {Id} */(idBuffer.subarray(1).toString('binary'));
	}
};
...
export default function FoundationdbAdapter(api, { db, name, revs_limit: revLimit }) {
	...
	const stores = {
		...
		docStore: /** @type {fdb.Subspace<Id | typeof startOfKeyspace | typeof endOfKeyspace, Id, Metadata, Metadata>} */(
			subspace.at(fdb.tuple.pack(DOC_STORE), idEncoder, fdb.encoders.json)
		),
		bySeqStore: /** @type {fdb.Subspace<number, number, Doc, Doc>} */(
	...
	api._bulkDocs = callbackify(
		...
		({ docs: userDocs }, opts) => doWriteTn(async tn => {
					...
					docInfo.data._attachments = Object.fromEntries(await Promise.all(
						Object.entries(docInfo.data._attachments).map(async ([key, att]) => {
							if (att.stub) {
								...
								const digest = /** @type {Digest} */(MD5_PREFIX + crypto.createHash('md5').update(data).digest('base64'));
			...
			/** @type {Record<Digest, Promise<boolean>>} */
	...
	api._allDocs = callbackify(
		 ...
		 *   update_seq?: boolean
		 * }} opts
		 * @returns {Promise<AllDocsResult>}
		 */
		opts => doReadTn(async tn => {
			/** @type {[AllDocsResultRow[], number, number]} */
	...
	*/
	 ...
	 *   attachments?: boolean,
	 *   binary?: boolean,
	 *   complete: (e?: null | Error, result?: ChangesResult) => void
	 *   continuous?: boolean,
	 *   descending?: boolean,
	 *   doc_ids?: Id[],
	 *   limit?: number,
	 *   onChange: (change: Change) => void,
	 *   processChange: (doc: Doc, metadata: Metadata, opts: unknown) => Change,
	 *   return_docs?: boolean,
	 *   since: number,
	 * }} opts
	 * @returns {{ cancel: () => void }}
	...
	api._changes = function (opts) {
		...
		iterate().then(
			v => {
				if (!cancelled) {
					opts.complete(null, /** @type {ChangesResult} */(v));
				}
			},
			e => {
				if (!cancelled) {
	 ...
	 *   since: number,
	 * }} opts
	 * @returns {AsyncGenerator<Change, undefined | ChangesResult>}
	 */
