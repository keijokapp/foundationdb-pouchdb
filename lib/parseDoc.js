...
/**
 * @import { PouchError } from 'pouchdb-errors'
 * @import {
 *   Doc,
 *   DocInfo,
 *   Id,
...
function parseRevisionInfo(rev) {
	...
	return parseRevision(/** @type {Rev} */(rev));
}

/**
 * @param {{ start: RevNum, ids: RevId[] }} revisions
 * @param {RevTreeNodeStatus} opts
 * @returns {{ pos: RevNum, ids: RevTreeNode }[]}
 */
function makeRevTreeFromRevisions(revisions, opts) {
	const pos = /** @type {RevNum} */(
...
export default function parseDoc(
	...
	if (newEdits) {
		if (metadata.id == null) {
			metadata.id = /** @type {Id} */(pouchdbUtils.uuid());
		}

		newRevId = pouchdbUtils.rev(
			!deterministicRevs || doc._id
				? /** @type {PartialBy<Doc, '_rev'>} */(doc)
				: { ...doc, _id: metadata.id },
			deterministicRevs
		);
		...
		if (metadata.rev != null) {
			...
			nRevNum = /** @type {RevNum} */(revInfo.prefix + 1);
		} else {
			metadata.rev_tree = [{
				pos: /** @type {RevNum} */(1),
				ids: [newRevId, opts, []]
			}];
			nRevNum = /** @type {RevNum} */(1);
		}
