/* eslint-disable stylistic/max-len */
import {
	collate,
	toIndexableString,
	normalizeKey,
	parseIndexableString
} from 'pouchdb-collate';
import createView from './createView.js';
import { handleNotFound } from './util.js';
import { mapper, reducer } from './evalFunction.js';

/**
 * @typedef {{
 *   get(id: string): Promise<import('../types.js').Doc | import('../types.js').LocalDoc>
 *   put(
 *     value:
 *       | ({ _id: `local/${import('../types.js').LocalId}`, _rev?: import('../types.js').LocalRev | undefined } & Record<string, any>)
 *       | ({ _id: import('../types.js').Id, _rev?: import('../types.js').Rev | undefined } & Record<string, any>)
 *   ): Promise<unknown>
 *   destroy(): Promise<void>
 *   allDocs(options: QueryOptions): Promise<{ total_rows: number, rows: unknown[] }>
 * }} PouchDB
 * @typedef {{
 *   db: PouchDB,
 *   mapFun: Function,
 *   name: string,
 *   reduceFun: Function,
 *   seq: number,
 *   sourceDB: PouchDB
 * }} View
 * @typedef {{
 *   _id: 'local/purges'
 *   purges: Array<{
 *     docId: import('../types.js').Id
 *     rev: import('../types.js').Rev
 *     purgeSeq: number
 *   }>
 *   purgeSeq: number
 * }} Purges
 * @typedef {{
 *   _id: 'local/purgeSeq'
 *   _rev: import('../types.js').LocalRev
 *   purgeSeq: number
 * }} PurgeSeq
 * @typedef {{
 *   attachments?: boolean | undefined
 *   binary?: boolean | undefined
 *   conflicts?: boolean | undefined
 *   deleted?: 'ok' | undefined
 *   descending?: boolean | undefined
 *   endkey?: import('../types.js').Id | undefined
 *   include_docs?: boolean | undefined
 *   inclusive_end?: boolean | undefined
 *   key?: import('../types.js').Id | undefined
 *   keys?: import('../types.js').Id[] | undefined
 *   limit?: number | undefined
 *   reduce?: boolean | undefined
 *   skip?: number | undefined
 *   startkey?: import('../types.js').Id | undefined
 *   update_seq?: boolean | undefined
 * }} QueryOptions
 */

class TaskQueue {
	constructor() {
		/** @type {Promise<unknown>} */
		this.promise = Promise.resolve();
	}

	/**
	* @template T
	* @param {() => Promise<Awaited<T>>} task
	* @returns {Promise<Awaited<T>>}
	*/
	add(task) {
		const promise = this.promise
			// just recover
			.catch(() => { })
			.then(() => task());

		this.promise = promise;

		return promise;
	}

	finish() {
		return this.promise;
	}
}

/** @type {Record<string, TaskQueue>} */
const queues = {};
const CHANGES_BATCH_SIZE = 50;

/**
* @param {string} name
* @returns {[string, string]}
*/
function parseViewName(name) {
	// can be either 'ddocname/viewname' or just 'viewname'
	// (where the ddoc name is the same)
	return name.indexOf('/') === -1
		? [name, name]
		: /** @type {any} */(name.split('/', 2));
}

/**
 * @param {Array<{rev: string}>} changes
 * @returns {boolean}
 */
function isGenOne(changes) {
	// only return true if the current change is 1-
	// and there are no other leafs
	return changes.length === 1 && /^1-/.test(changes[0].rev);
}

/**
 * @template T
 * @param {T[]} array
 * @returns {T[]}
 */
function uniq(array) {
  var theSet = new Set(array);
  var result = new Array(theSet.size);
  var index = -1;
  theSet.forEach(function (value) {
    result[++index] = value;
  });
  return result;
}

/**
 * @template T
 * @param {Map<T, unknown>} map
 * @returns {T[]}
 */
function mapToKeysArray(map) {
  var result = new Array(map.size);
  var index = -1;
  map.forEach(function (value, key) {
    result[++index] = key;
  });
  return result;
}

/**
 * @param {{ key: unknown, value: unknown }} x
 * @param {{ key: unknown, value: unknown }} y
 * @returns {number}
 */
function sortByKeyThenValue(x, y) {
	return collate(x.key, y.key) || collate(x.value, y.value);
}

/**
 * @template T
 * @param {T[]} results
 * @param {number | undefined} limit
 * @param {number | undefined} skip
 * @returns {T[]}
 */
function sliceResults(results, limit, skip) {
	skip = skip != null && Number.isFinite(skip) && skip > 0 ? skip : 0;

	if (limit != null && Number.isFinite(limit)) {
		return results.slice(skip, limit + skip);
	}

	if (skip > 0) {
		return results.slice(skip);
	}

	return results;
}

/**
 * @param {{ id: string, value: unknown }} row
 * @returns {string}
 */
function rowToDocId({ id, value }) {
	// Users can explicitly specify a joined doc _id, or it
	// defaults to the doc _id that emitted the key/value.
	return /** @type {string} */((value && typeof value === 'object' && '_id' in value && value._id) ?? id);
}

// returns a promise for a list of docs to update, based on the input docId.
// the order doesn't matter, because post-3.2.0, bulkDocs
// is an atomic operation in all three adapters.
async function getDocsToPersist(docId, view, docIdsToChangesAndEmits) {
	const metaDocId = `_local/doc_${docId}`;
	const defaultMetaDoc = { _id: metaDocId, keys: [] };
	const docData = docIdsToChangesAndEmits.get(docId);
	const indexableKeysToKeyValues = docData[0];
	const changes = docData[1];

	const metaDoc = isGenOne(changes)
		? defaultMetaDoc
		: await view.db.get(metaDocId).catch(handleNotFound(defaultMetaDoc));

	const keyValueDocs = metaDoc.keys.length
		? await view.db.allDocs({
			keys: metaDoc.keys,
			include_docs: true
		})
		: { rows: [] };

	const kvDocs = [];
	const oldKeys = new Set();

	for (const row of keyValueDocs.rows) {
		const { doc } = row;

		if (!doc) { // deleted
			continue;
		}

		kvDocs.push(doc);
		oldKeys.add(doc._id);
		doc._deleted = !indexableKeysToKeyValues.has(doc._id);

		if (!doc._deleted) {
			const keyValue = indexableKeysToKeyValues.get(doc._id);

			if ('value' in keyValue) {
				doc.value = keyValue.value;
			}
		}
	}

	const newKeys = mapToKeysArray(indexableKeysToKeyValues);

	for (const key of newKeys) {
		if (!oldKeys.has(key)) {
			// new doc
			const kvDoc = {
				_id: key
			};
			const keyValue = indexableKeysToKeyValues.get(key);

			if ('value' in keyValue) {
				kvDoc.value = keyValue.value;
			}

			kvDocs.push(kvDoc);
		}
	}

	metaDoc.keys = uniq(newKeys.concat(metaDoc.keys);
	kvDocs.push(metaDoc);

	return kvDocs;
}

/**
 * @param {View} view
 */
async function updatePurgeSeq(view) {
	try {
		const result = /** @type {Purges} */(await view.sourceDB.get('_local/purges'));

		const { purgeSeq } = result;

		const res = /** @type {PurgeSeq | undefined} */(await view.db.get('_local/purgeSeq').catch(handleNotFound()));

		await view.db.put({
			_id: /** @type {`local/${import('../types.js').LocalId}`} */(`_local/${purgeSeq}`),
			_rev: res?._rev,
			purgeSeq
		});
	} catch (/** @type {any} */e) {
		if (e.status !== 404) {
			throw e;
		}
	}
}

/**
 * updates all emitted key/value docs and metaDocs in the mrview database
 * for the given batch of documents from the source database
 * @param {View} view
 * @param {Map<any, unknown>} docIdsToChangesAndEmits
 * @param {number} seq
 * @returns {Promise<void>}
 */
async function saveKeyValues(view, docIdsToChangesAndEmits, seq) {
	const lastSeqDoc = await view.db.get('_local/lastSeq').catch(handleNotFound());

	const docIds = mapToKeysArray(docIdsToChangesAndEmits);

	const listOfDocsToPersist = await Promise.all(docIds.map(docId => getDocsToPersist(docId, view, docIdsToChangesAndEmits)));

	const docsToPersist = listOfDocsToPersist.flat();
	lastSeqDoc.seq = seq;
	docsToPersist.push({
		...lastSeqDoc,
		_id: '_local/lastSeq',
		seq
	});

	await view.db.bulkDocs({ docs: docsToPersist });

	await updatePurgeSeq(view);
}

/**
 * @param {string} viewName
 * @returns {TaskQueue}
 */
function getQueue(viewName) {
	queues[viewName] ??= new TaskQueue();

	return queues[viewName];
}

/**
 * @param {View} view
 */
async function updateView(view) {
	let currentSeq = view.seq ?? 0;

	const mapFun = mapper(view.mapFun);

	/**
	 * @param {Map<import("../types.js").Id, [unknown, unknown[]]>} docIdsToChangesAndEmits
	 * @param {number} seq
	 * @returns {() => Promise<void>}
	 */
	function processChange(docIdsToChangesAndEmits, seq) {
		return function () {
			return saveKeyValues(view, docIdsToChangesAndEmits, seq);
		};
	}

	const queue = new TaskQueue();

	async function processBatch() {
		const { results } = await view.sourceDB.changes({
			return_docs: true,
			conflicts: true,
			include_docs: true,
			style: 'all_docs',
			since: currentSeq,
			limit: CHANGES_BATCH_SIZE
		});

		const [purge, purges] = await Promise.all([
			/** @type {Promise<PurgeSeq | undefined>} */(view.db.get('_local/purgeSeq').catch(handleNotFound())),
			/** @type {Promise<Purges | undefined>} */(view.sourceDB.get('_local/purges').catch(handleNotFound()))
		]);

		if (purges != null) {
			const purgeSeq = purge != null ? purge.purgeSeq : -1;

			results.push(
				...(await Promise.all(
					purges.purges
						.filter((purge, index) => index > purgeSeq)
						.map(purge => purge.docId)
						.filter((docId, index, arr) => arr.indexOf(docId) === index)
						.map(async docId => {
							const doc = await view.sourceDB.get(docId).catch(handleNotFound());

							return { docId, doc };
						})
				))
					.filter(purge => results.every(change => change.id !== purge.docId))
					.map(purge => ({
						_id: purge.docId,
						doc: purge.doc ?? { _id: purge.docId, _deleted: 1 },
						changes: purge.doc ? [{ rev: purge.doc._rev }] : []
					}))
			);
		}

		if (!results.length) {
			return;
		}

		const docIdsToChangesAndEmits = createDocIdsToChangesAndEmits(results);

		queue.add(processChange(docIdsToChangesAndEmits, currentSeq));

		if (results.length < CHANGES_BATCH_SIZE) {
			return;
		}

		return processBatch();
	}

	function createDocIdsToChangesAndEmits(results) {
		/** @type {Map<import('../types.js').Id, [unknown, unknown[]]>} */
		const docIdsToChangesAndEmits = new Map();

		for (const change of results) {
			if (change.doc._id[0] !== '_') {
				const { doc } = change;

				const mapResults = doc._deleted
					? []
					: mapFun(doc).map(([key, value]) => ({
						id: doc._id,
						key: normalizeKey(key),
						value: value != null
							? normalizeKey(value)
							: undefined
					}));

				mapResults.sort(sortByKeyThenValue);

				const indexableKeysToKeyValues = new Map();
				let lastKey;

				for (let i = 0, { length } = mapResults; i < length; i++) {
					const emittedKeyValue = mapResults[i];
					const complexKey = [emittedKeyValue.key, emittedKeyValue.id];

					if (i > 0 && collate(emittedKeyValue.key, lastKey) === 0) {
						complexKey.push(i); // dup key+id, so make it unique
					}

					indexableKeysToKeyValues.set(toIndexableString(complexKey), emittedKeyValue);
					lastKey = emittedKeyValue.key;
				}

				docIdsToChangesAndEmits.set(change.doc._id, [
					indexableKeysToKeyValues,
					change.changes
				]);
			}

			currentSeq = change.seq;
		}

		return docIdsToChangesAndEmits;
	}

	await processBatch();
	await queue.finish();
	view.seq = currentSeq;
}

function reduceView(view, results, options) {
	if (options.group_level === 0) {
		delete options.group_level;
	}

	const shouldGroup = options.group || options.group_level;
	const reduceFun = reducer(view.reduceFun);
	const groups = [];
	const lvl = !Number.isFinite(options.group_level)
		? Number.POSITIVE_INFINITY
		: options.group_level;

	for (const result of results) {
		const last = groups[groups.length - 1];
		let groupKey = shouldGroup ? result.key : null;

		// only set group_level for array keys
		if (shouldGroup && Array.isArray(groupKey)) {
			groupKey = groupKey.slice(0, lvl);
		}

		if (last && collate(last.groupKey, groupKey) === 0) {
			last.keys.push([result.key, result.id]);
			last.values.push(result.value);
			continue;
		}

		groups.push({
			keys: [[result.key, result.id]],
			values: [result.value],
			groupKey
		});
	}

	results = groups.map(group => ({
		value: reduceFun(group.keys, group.values, false),
		key: group.groupKey
	}));

	return { rows: sliceResults(results, options.limit, options.skip) };
}

/**
 * @param {View} view
 * @param {QueryOptions} opts
 * @returns
 */
async function queryViewInQueue(view, opts) {
	const shouldReduce = view.reduceFun && opts.reduce !== false;
	const skip = opts.skip || 0;

	if (typeof opts.keys !== 'undefined' && !opts.keys.length) {
		// equivalent query
		opts.limit = 0;
		delete opts.keys;
	}

	/** @type {QueryOptions[]} */
	let queries;

	if (typeof opts.keys !== 'undefined') {
		queries = opts.keys.map(key => ({
			startkey: toIndexableString([key]),
			endkey: toIndexableString([key, {}]),
			include_docs: true,
			update_seq: opts.update_seq
		}));
	} else {
	/** @type {QueryOptions} */
		const viewOptions = {
			descending: opts.descending,
			include_docs: true,
			update_seq: opts.update_seq
		};

		if (typeof opts.key !== 'undefined') {
			const keyStart = toIndexableString([opts.key]);
			const keyEnd = toIndexableString([opts.key, {}]);

			if (viewOptions.descending) {
				viewOptions.endkey = keyStart;
				viewOptions.startkey = keyEnd;
			} else {
				viewOptions.startkey = keyStart;
				viewOptions.endkey = keyEnd;
			}
		} else {
			if (typeof opts.startkey !== 'undefined') {
				viewOptions.startkey = viewOptions.descending
					? toIndexableString([opts.startkey, {}])
					: toIndexableString([opts.startkey]);
			}

			if (typeof opts.endkey !== 'undefined') {
				let inclusiveEnd = opts.inclusive_end !== false;

				if (opts.descending) {
					inclusiveEnd = !inclusiveEnd;
				}

				viewOptions.endkey = toIndexableString(
					inclusiveEnd ? [opts.endkey, {}] : [opts.endkey]
				);
			}
		}

		if (!shouldReduce) {
			if (typeof opts.limit === 'number') {
				viewOptions.limit = opts.limit;
			}

			viewOptions.skip = skip;
		}

		queries = [viewOptions];
	}

	let totalRows = 0;

	const results = await Promise.all(queries.map(async query => {
		const result = await view.db.allDocs(query);

		totalRows += result.total_rows;

		return result.rows;
	}));

	let rows = results.flat().map(result => {
		const parsedKeyAndDocId = parseIndexableString(result.doc._id);

		return {
			key: parsedKeyAndDocId[0],
			id: parsedKeyAndDocId[1],
			value: 'value' in result.doc ? result.doc.value : null
		};
	});

	let finalResults;

	if (shouldReduce) {
		const finalResults = reduceView(view, rows, opts);

		if (opts.update_seq) {
			finalResults.update_seq = view.seq;
		}

		return finalResults;
	}

	if (typeof opts.keys === 'undefined') {
		finalResults = {
			total_rows: totalRows,
			offset: skip,
			rows
		};
	} else {
		rows = sliceResults(rows, opts.limit, opts.skip);

		finalResults = {
			total_rows: totalRows,
			offset: skip,
			rows
		};
	}

	if (opts.update_seq) {
		finalResults.update_seq = view.seq;
	}

	if (opts.include_docs) {
		const allDocsResult = await view.sourceDB.allDocs({
			keys: uniq(rows.map(rowToDocId)),
			include_docs: true,
			conflicts: opts.conflicts,
			attachments: opts.attachments,
			binary: opts.binary
		});

		const allDocs = Object.fromEntries(allDocsResult.rows.map(row => [row.id, row.doc]));

		rows.forEach(row => {
			const id = rowToDocId(row);

			if (id in allDocs) {
				row.doc = allDocs[id];
			}
		});
	}

	return finalResults;
}

/**
 * @param {PouchDB} db
 * @param {{ map: Function, reduce?: Function } | string} fun
 * @param {object} opts
 */
export async function query(db, fun, opts) {
	if (typeof fun !== 'string') {
		return getQueue('temp_view/temp_view').add(async () => {
			const { map, reduce } = typeof fun === 'function'
				? { map: fun }
				: fun;

			const view = await createView(db, 'temp_view/temp_view', map, reduce);

			try {
				await updateView(view);

				return await queryViewInQueue(view, opts);
			} finally {
				await view.db.destroy();
			}
		});
	}

	const [designDocName, viewName] = parseViewName(fun);
	const fullViewName = `${designDocName}/${viewName}`;

	return getQueue(fullViewName).add(async () => {
		const doc = await db.get(`_design/${designDocName}`);

		if (!doc.views[viewName]) {
			throw new Error(`ddoc ${doc._id} has no view named ${viewName}`);
		}

		const { map, reduce } = doc.views[viewName];

		const view = await createView(db, fullViewName, map, reduce);

		await updateView(view);

		return queryViewInQueue(view, opts);
	});
}

/**
 * @this {PouchDB}
 * @returns {Promise<{ ok: true }>}
 */
export async function viewCleanup() {
	try {
		const metaDoc = await this.get('_local/mrviews');
		const docsToViews = new Map();

		for (const fullViewName of Object.keys(metaDoc.views)) {
			const parts = parseViewName(fullViewName);
			const designDocName = `_design/${parts[0]}`;
			const viewName = parts[1];
			let views = docsToViews.get(designDocName);

			if (!views) {
				views = new Set();
				docsToViews.set(designDocName, views);
			}

			views.add(viewName);
		}

		const opts = {
			keys: mapToKeysArray(docsToViews),
			include_docs: true
		};

		const res = await this.allDocs(opts);
		/** @type {Record<string, boolean>} */
		const viewsToStatus = {};

		for (const row of res.rows) {
			const ddocName = row.key.substring(8); // cuts off '_design/'

			for (const viewName of docsToViews.get(row.key)) {
				let fullViewName = `${ddocName}/${viewName}`;

				/* istanbul ignore if */
				if (!metaDoc.views[fullViewName]) {
					// new format, without slashes, to support PouchDB 2.2.0
					// migration test in pouchdb's browser.migration.js verifies this
					fullViewName = viewName;
				}

				const viewDBNames = Object.keys(metaDoc.views[fullViewName]);
				// design doc deleted, or view function nonexistent
				const statusIsGood = row.doc && row.doc.views && row.doc.views[viewName];

				for (const viewDBName of viewDBNames) {
					viewsToStatus[viewDBName] ??= !!statusIsGood;
				}
			}
		}

		const dbsToDelete = Object.keys(viewsToStatus).filter(viewDBName => !viewsToStatus[viewDBName]);

		const destroyPromises = dbsToDelete.map(viewDBName => getQueue(viewDBName).add(() => new this.constructor(viewDBName, this.__opts).destroy()));

		await Promise.all(destroyPromises);
	} catch (/** @type {any} */err) {
		if (err.status !== 404) {
			throw err;
		}
	}

	return { ok: true };
}
