 ...
 * @returns {{ docInfo: DocInfo, delta: number } | undefined}
 */
 ...
 * @returns {{ docInfo: DocInfo, delta: number }}
 */
...
function transformDoc(
	...
	if (docInfo.data._attachments) {
		const attachments = Object.entries(docInfo.data._attachments);
		docInfo.attachments = [];
		docInfo.data._attachments = {};

		attachments.forEach(([key, att]) => {
			...
			if (!att.stub) {
				...
				const digest = /** @type {Digest} */(`md5-${crypto.createHash('md5').update(data).digest('base64')}`);
