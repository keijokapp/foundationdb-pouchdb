 ...
 * @param {(Array<LocalDoc | DocInfo>)} docInfos
 * @param {Record<
 *   '_removeLocal' | '_putLocal',
 *   (
 *     doc: LocalDoc,
 *     opts: { ctx: Transaction },
 *     callback: (e?: Error, result?: BulkDocsResultRow) => void
 *   ) => void
 * >} api
 * @param {Map<Id, Metadata>} fetchedDocs
 ...
 *   isUpdate: boolean,
 *   delta: number
 * ) => Promise<BulkDocsResultRow>} writeDoc
...
async function handleDoc(fetchedDocs, id, docInfo, revLimit, writeDoc, opts) {
	...
	if (fetchedDocs.has(id)) {
		const prev = /** @type {Metadata} */(fetchedDocs.get(id));
		...
		if (previouslyDeleted && !deleted && newEdits && isRoot) {
			...
			docInfo = /** @type {DocInfo} */(parseDoc(newDoc, newEdits));
		}
