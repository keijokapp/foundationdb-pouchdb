...
export default function FoundationdbAdapter(api, db) {
	...
	api._get = callbackify(
		 ...
		 *   entry: import('./types.js').Metadata
		 *   record: import('./types.js').Doc
		 * }>}
		 */
		id => doReadTn(async tn => {
			const entry = await getDoc(id, tn);

			if (entry == null) {
				return;
			}

			const rev = getWinningRev(entry);
			const deleted = getIsDeleted(entry, rev);
			...
			if (entry.rev_map[rev] == null) {
				return;
			}

			const record = await getBySeq(entry.rev_map[rev], tn);
	...
	api._allDocs = callbackify(
		 ...
		 */
		opts => doReadTn(tn => getAllDocsRows(opts, tn))
	);
