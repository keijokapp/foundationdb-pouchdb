...
import { compactTree, traverseRevTree } from 'pouchdb-merge';
...
export default function FoundationdbAdapter(api, db) {
	...
	function getDoc(id, tn) {
		...
		if (state.sparse.docStore.has(id)) {
	...
	function getBySeq(seq, tn) {
		...
		assert(state.sparse.bySeqStore.has(seq));
	...
	api._bulkDocs = (userDocs, opts) => doWriteTn(async tn => {
		...
		const results = await Promise.allSettled(docInfos.map(async docInfo => {
			...
			const result = existingDoc
				? handleUpdatedDoc(existingDoc, docInfo, 1000, opts)
				: handleNewDoc(docInfo, 1000, opts);
	 ...
	 * @returns {Promise<
	 *   | { id: import('./types.js').Id, doc: import('./types.js').Doc }
	 *   | undefined
	 * >}
	 */
	api.get = key => doReadTn(async tn => {
		...
		if (metadata != null) {
			return {
				id: metadata.id,
				doc: await nonNullPromise(getBySeq(metadata.seq, tn))
			};
		}
	});
	 ...
	 *   id: import('./types.js').Id
	 *   doc: import('./types.js').Doc | undefined
	 * }>>}
	 */
	api.getRange = opts => doReadTn(async tn => {
		...
		const state = getTransactionState(tn, name);
		const pendingDocs = [...state.sparse.docStore.values()]
			.filter(
		...
		for (;;) {
			...
			const it = tn.at(stores.docStore).getRange(iterationStart, iterationEnd, { reverse });
			...
			for await (const [, metadata] of it) {
				...
				results.push(processRow(metadata, tn, includeDocs));
			}

			if (pendingDocIndex < pendingDocs.length) {
				const metadata = pendingDocs[pendingDocIndex];
	   ...
	   * @returns {Promise<{
		 ...
		 * }>}
	   */
		async function processRow(metadata, tn, includeDocs) {
			return {
				id: metadata.id,
				doc: includeDocs ? await nonNullPromise(getBySeq(metadata.seq, tn)) : undefined
			};
		}
	});
